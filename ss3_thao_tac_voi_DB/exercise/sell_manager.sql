...
select customer.c_name as"khách hàng đã mua hàng" from customer 
join `order` on customer.c_id = `order`.c_id ;

select product.p_name as"sản phẩm được mua" from product
join order_detail on product.p_id = order_detail.p_id;

-- Hiển thị tên những khách hàng không mua bất kỳ một sản phẩm nào
select customer.c_id, customer.c_name as "khách hành không mua hàng" from customer where customer.c_name not in (select customer.c_name as"khách hàng đã mua hàng" from customer 
join `order` on customer.c_id = `order`.c_id );